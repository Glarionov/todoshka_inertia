...
let onlineUsers = ref(props.initialOnlineUsers.data);
...
const loadMessages = async (initialLoad = false) => {
    ...
    if (response.data.data) {
        setTimeout(() => {blockLoading = false}, 500);
    }
}

const sendMessage = async () => {
    // request.post(route('messages.store'), form, "Can't save message");
...
<template>
    <ChatLayout>
        <div class="text-yellow-300 bg-gray-500 fixed w-full p-1 xl:hidden flex justify-end">
            <div class="p-1 cursor-pointer" @click="showSidebarOnNarrowScreen = !showSidebarOnNarrowScreen">
                Show menu
            </div>
        </div>
        <div class="p-4 w-60 absolute bg-gray-700 m-4 xl:inline-block" :class="{'hidden': !showSidebarOnNarrowScreen}">
            ...
            <div v-for="(onlineUser, index) in onlineUsers" :key="index" class="p-1 text-yellow-300">
                {{onlineUser.name}}
            </div>
        </div>
                ...
                <div class="p-2 messages__box">
                    <div class="chat-message">
                        <div class="flex items-end" :class="{'justify-end': message.own}">
                            <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 items-end" :class="[message.own? 'order-1': 'order-2']">
                                <div><div class="px-4 py-2 rounded-lg inline-block break-all" :class="[message.own? 'bg-blue-600': 'bg-green-800']">
                            ...
                            <img src="https://images.unsplash.com/photo-1590031905470-a1a1feacbb0b?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=facearea&amp;facepad=3&amp;w=144&amp;h=144"
                                 alt="My profile" class="w-6 h-6 rounded-full" :class="[message.own? 'order-2': 'order-1']">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <form class="max-w-screen-md m-auto bg-gray-800 send-message-form m-auto" @submit.prevent="sendMessage">
            <div class="flex items-center h-full p-2">
                <div v-if="form.errors.text" class="text-red-400">{{ form.errors.text }}</div>
                <textarea class="appearance-none bg-transparent w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none h-full resize-none text-white"
                          v-model="form.text"
